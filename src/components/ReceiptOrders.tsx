...
const ReceiptOrders = ({ status }: { status: string | null }) => {
  const { recieptTasks, userInfo } = useContext(Context);
  ...
  return (
    <div className="px-4">
      ...
      {status === "Waiting" && filteredTasks.length > 0 && (
         <div className="flex items-center justify-between py-2 px-3 border-b-2 border-gray-500 border-l-0 border-r-0">
